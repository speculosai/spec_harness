 ...
 *
 * Approve refund writes the decision to the return record. The chip beside it is read
 * back from that record on the next pass, never set here.
 */

import { money, plural } from '../lib/format';
import { isWaiting } from '../lib/records';
...
export function RefundQueue({
  ...
  entries: ReturnRow[];
  busyId: string | null;
  onApprove: (entry: ReturnRow) => void;
}) {
  if (!entries.length) {
          ...
          {entries.map((entry) => (
            <tr key={entry.id}>
              ...
              <td className="px-3 py-2">
                <Chip tone={isWaiting(entry) ? 'waiting' : 'done'}>{entry.refundStatus}</Chip>
              </td>
              <td className="px-3 py-2 text-right">
                {isWaiting(entry) ? (
                  <ActionButton busy={busyId === entry.id} onClick={() => onApprove(entry)}>
                    Approve refund
                  </ActionButton>
                ) : (
                  <span className="text-xs text-stone-500">Money on its way</span>
                )}
              </td>
            </tr>
          ))}
