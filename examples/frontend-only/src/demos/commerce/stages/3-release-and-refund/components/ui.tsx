 ...
 * the records will not come, the chip that marks a row's state, and the button that
 * changes a record.
 *
...
/**
 * The button that writes back.
 *
 * It disables itself while the decision is in flight, so a slow answer cannot be sent
 * twice, and it says what it is doing rather than spinning in silence.
 */
export function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={busy}
      className="rounded-md border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-800 hover:bg-indigo-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 disabled:border-stone-200 disabled:bg-stone-50 disabled:text-stone-400"
    >
