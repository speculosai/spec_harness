/**
 * The attention board, the late-payment panel, and the button that chases one.
 *
 * The board answers "what needs a look today"; the panel answers the first question
 * anyone asks after seeing it - how far behind, and which building is carrying it - and
 * now ends the job rather than describing it.
 *
 * The page still owns the reading, and that is what makes the last part work: when a
 * reminder goes out, the panel asks for `load` again and every figure on the screen is
 * recomputed from what the records now say.
 */
...
import { readRecords } from './lib/data';
...
export function App() {
  ...
  const charges = records ? lateCharges(records) : [];
  const owed = sumOwed(charges);
        ...
        <div className="mt-5">
          {records === null ? (
            ...
            <div className="space-y-4">
              ...
              <LatePayments charges={charges} onReminderSent={load} />
            </div>
          )}
        </div>
      </div>
