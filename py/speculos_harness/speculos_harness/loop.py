...
_NOT_IMPLEMENTED = "speculos_harness.loop: implementation pending"
...
async def run_turn(
    *,
    ...
    TODO: implement the tool-calling loop described in the module
    docstring. Yields ``event: <name>\\ndata: <json>\\n\\n`` frames ready to
    write to the streaming response.
    """
    raise NotImplementedError(_NOT_IMPLEMENTED)
    # Unreachable; marks this as an async generator so the declared
    # AsyncIterator[bytes] signature holds.
...
async def execute_tool(
    ...
    TODO: call ``tool.execute(args, ctx)``, coerce to the
...
def build_tool_registry(
    ...
    TODO: the built-in file/package tools plus connector-contributed and
...
def initial_messages(
    ...
    TODO: system prompt + sanitized history + the new user message
