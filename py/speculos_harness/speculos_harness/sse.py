...
What lands here at v0.1: the ``sse_frame`` encoder and one thin emitter per
event. Every function is a stub until the v0.1 code drop.
"""
...
def sse_frame(event: str, data: Mapping[str, Any]) -> bytes:
    ...
    TODO(v0.1): validate ``event`` against :data:`EVENT_NAMES`, JSON-encode
...
# ``json`` is imported so the v0.1 encoder has it in scope; referenced here to
# keep linters quiet without changing the stub behavior.
_ = json
