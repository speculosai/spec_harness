...
_NOT_IMPLEMENTED = "speculos_harness.stores: implementation pending"
...
class _BaseProjectStore(ProjectStore):
    ...
    async def put_files(self, id: str, files: FileMap) -> None:
        """Full-replace, transactional. TODO: swap the file map atomically."""
        raise NotImplementedError(_NOT_IMPLEMENTED)
    ...
    async def create_snapshot(self, id: str, s: Mapping[str, Any]) -> Snapshot:
        """TODO: persist a snapshot; prune pre-turn ones to SNAPSHOT_KEEP."""
        raise NotImplementedError(_NOT_IMPLEMENTED)
...
class SQLiteProjectStore(_BaseProjectStore):
    ...
    def __init__(self, path: str = "./harness.db") -> None:
        self.path = path
        # TODO: open/create the database, run migrations for the
...
class FsProjectStore(_BaseProjectStore):
    ...
    def __init__(self, root: str = "./harness-projects") -> None:
        self.root = root
        # TODO: ensure the root exists; lay out one directory per project.
