...
_NOT_IMPLEMENTED = "speculos_harness.tools.files: implementation pending"
...
#: raises to reject the write. TODO: thread through write_file/edit_file.
...
class _FileTool(AgentTool):
    ...
    def available(self, ctx: ToolContext) -> bool:
        """Offered by default. TODO: override where a capability gates it."""
...
class WriteFileTool(_FileTool):
    ...
    TODO: read the file map, set ``path`` to ``content`` (through the
...
class EditFileTool(_FileTool):
    ...
    TODO: locate the single match, apply ``replace`` (through the optional
...
class ReadFileTool(_FileTool):
    ...
    TODO: return the file's text, or an error result when it is absent.
    """
...
class DeleteFileTool(_FileTool):
    ...
    TODO: drop ``path`` from the file map and full-replace back.
    """
...
class InstallPackageTool(_FileTool):
    ...
    TODO: call the bundler's installer (always ``--ignore-scripts``,
...
def builtin_file_tools(
    *, write_validator: Optional[WriteValidator] = None
    ...
    TODO: return concrete instances with real schemas and executors. The
