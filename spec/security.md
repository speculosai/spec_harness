...
what the system defends against, how, and what residual risk remains. Several
invariants here are non-negotiable, and the reference implementations carry a
startup self-check that refuses to run when one is violated.
...
- **A curated dependency allowlist** caps what `install_package` may add, so a
  coerced install of an attacker-chosen package simply fails. A host configures
  the list; it is the direct backstop for this threat.
